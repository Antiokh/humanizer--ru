...
name: humanizer--ru
license: MIT
description: Редактор русского текста - убирает признаки AI-генерации, канцелярит и воду, сохраняя факты и добавляя голос. В отличие от smixs/humanizer-ru, сохраняет нормативное длинное тире и отмечает только его шаблонный переизбыток как WARN. 34 паттерна AI-слопа с жёсткими запретами (негативные параллелизмы, математические знаки, техножаргон) и детерминированным линтером. Use when пользователь просит "отредактируй", "убери воду", "сделай живым", "humanize", "очисти от AI", "перепиши человечнее", говорит что текст "звучит как нейросеть", или готовит AI-черновик к публикации. Do NOT use for написания текста с нуля, перевода, проверки орфографии и пунктуации, нерусского текста, кода и конфигов.
---
...
7. **Запусти линтер по чистовику** (без changelog и цитат «до»):
   ...
   - `WARN` - маркер паттерна или эвристика: оцени кластерами; одиночное «однако» или одно нормативное тире ничего не значит.
...
| Тире в каждом втором предложении | механическая замена всех тире на дефисы, двоеточия или точки | сохранить нормативные тире, а лишние конструкции переписать по смыслу |
...
- Внешние кавычки «ёлочки» вместо "технических"; если внутри цитаты нужны вложенные кавычки, используй „немецкие лапки“.
- Длинное тире «—» используй по нормам русской пунктуации. Не заменяй его дефисом «-» и не ставь механически в каждом втором предложении.
- Минимум жирного - только где реально важно
- В заголовках стандартная капитализация, как в обычном предложении: с заглавной буквы только первое слово и имена собственные.
...
3. В резюме - только то, что реально сделано и реально посчитано: не заявляй прогонов линтера и проверок, которых не было, не приводи цифр, которых не пересчитывал. Резюме держит те же нормы стиля, что чистовик: без стрелок, математических знаков и прочего из жёстких запретов
